...
<div class="table_box">
  ...
  $fetch = "SELECT * FROM inventory WHERE brand_name LIKE '%$brand_name' order by expiration ASC";
  $result3 = $connect->query($fetch);
  }
  else{
  $message="";
  $fetch = "SELECT * FROM inventory ORDER BY expiration ASC"; 
  $result3 = $connect->query($fetch);
}
    if($result3->num_rows){
...
<?php 
    ...
    <tr>
      ...
      <td><a href="admin_edit_med.php?product_id=<?php echo $row['product_id'] ?>"><img src="../image/edit.png" title="Edit Info"></a><a href="../php/med_delete.php?product_id=<?php echo $row['product_id']?>"><img src="../image/delete1.png" title="Delete Info" onclick="return confirm('Are you sure you want to delete this medicine')"></a></td>
        ...
        echo"<script>alert('No Results Found');window.location.href=('admin_products.php');</script>";
    }
    else{
      echo"<div class='message'>
            <label>No Added Medicine</label>
          </div>";
    }
  ?>
  </table>
